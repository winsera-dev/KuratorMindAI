...
```bash
node "/Users/muflihiskandar/Products - Winsera/kuratormind-ai/.agent/skills/gsd/get-shit-done/bin/gsd-tools.cjs" validate health $REPAIR_FLAG
```
...
```bash
node "/Users/muflihiskandar/Products - Winsera/kuratormind-ai/.agent/skills/gsd/get-shit-done/bin/gsd-tools.cjs" validate health
```
...
TASKS_DIR="/Users/muflihiskandar/Products - Winsera/kuratormind-ai/.agent/skills/gsd/tasks"
if [ -d "$TASKS_DIR" ]; then
  STALE_COUNT=$( (find "$TASKS_DIR" -maxdepth 1 -type d -mtime +1 2>/dev/null || true) | wc -l )
  if [ "$STALE_COUNT" -gt 0 ]; then
    echo "⚠️  Found $STALE_COUNT stale task directories in /Users/muflihiskandar/Products - Winsera/kuratormind-ai/.agent/skills/gsd/tasks/"
    echo "   These are leftover from crashed subagent sessions."
    echo "   Run: rm -rf /Users/muflihiskandar/Products - Winsera/kuratormind-ai/.agent/skills/gsd/tasks/*  (safe — only affects dead sessions)"
  fi
fi
```
