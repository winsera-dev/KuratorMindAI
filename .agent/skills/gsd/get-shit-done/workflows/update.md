...
- Path contains `/.agent/skills/gsd/` -> `gemini`
...
RUNTIME_DIRS=( "claude:.claude" "opencode:.config/opencode" "opencode:.opencode" "gemini:.agent/skills/gsd" "kilo:.config/kilo" "kilo:.kilo" "codex:.codex" )
ENV_RUNTIME_DIRS=()
...
if [ -n "$PREFERRED_CONFIG_DIR" ] && { [ -f "$PREFERRED_CONFIG_DIR/get-shit-done/VERSION" ] || [ -f "$PREFERRED_CONFIG_DIR/get-shit-done/workflows/update.md" ]; }; then
  ...
  for dir in .claude .config/opencode .opencode .agent/skills/gsd .config/kilo .kilo .codex; do
...
global: `/Users/muflihiskandar/Products - Winsera/kuratormind-ai/.agent/skills/gsd/`, `~/.config/opencode/`, `~/.opencode/`, `~/.agent/skills/gsd/`, `~/.config/kilo/`, or `~/.codex/`
local: `./.agent/skills/gsd/`, `./.config/opencode/`, `./.opencode/`, `./.agent/skills/gsd/`, `./.kilo/`, or `./.codex/`)
