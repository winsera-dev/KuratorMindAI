...
# T-22: CORS Configuration — Dynamic origins based on ENVIRONMENT
# Localhost is only permitted in development to prevent cross-site leaks in production.
_ALLOWED_ORIGINS = [
    "https://kuratormind-web-198564174902.asia-southeast1.run.app",
]

if os.getenv("ENVIRONMENT") == "development" or os.getenv("AUTH_ENABLED", "true").lower() == "false":
...
# T-21: Security Headers Middleware
# Adds standard forensic-grade transport protections.
@app.middleware("http")
async def add_security_headers(request: Request, call_next):
    ...
    # Basic Content-Security-Policy to restrict source of code/data
    response.headers["Content-Security-Policy"] = "default-src 'self'; script-src 'self' 'unsafe-inline'; style-src 'self' 'unsafe-inline'; img-src 'self' data:; connect-src 'self' *.supabase.co *.googleapis.com;"
    return response
