...
-- Vaults: Core workspace container for a bankruptcy case
...
-- Vault Documents: Metadata for uploaded forensic files
...
BEGIN
    ...
    WHERE dc.vault_id = match_vault_id
...
CREATE POLICY vault_owner ON public.vaults
    FOR ALL USING (auth.uid() = user_id);

-- Policy: Users can see documents in vaults they own
CREATE POLICY vault_docs_access ON public.vault_documents
    FOR ALL USING (
        vault_id IN (SELECT id FROM public.vaults WHERE user_id = auth.uid())
    );

-- Policy: Users can see document chunks in vaults they own
CREATE POLICY vault_chunks_access ON public.document_chunks
    FOR ALL USING (
        vault_id IN (SELECT id FROM public.vaults WHERE user_id = auth.uid())
    );

-- Policy: Users can see sessions in vaults they own
CREATE POLICY vault_sessions_access ON public.chat_sessions
    FOR ALL USING (
        vault_id IN (SELECT id FROM public.vaults WHERE user_id = auth.uid())
    );

-- Policy: Users can see messages in sessions belonging to vaults they own
CREATE POLICY vault_messages_access ON public.chat_messages
    FOR ALL USING (
        session_id IN (
            SELECT cs.id FROM public.chat_sessions cs
            JOIN public.vaults v ON cs.vault_id = v.id
            WHERE v.user_id = auth.uid()
        )
    );

-- Policy: Users can see claims in vaults they own
CREATE POLICY vault_claims_access ON public.claims
    FOR ALL USING (
        vault_id IN (SELECT id FROM public.vaults WHERE user_id = auth.uid())
    );

-- Policy: Users can see audit flags in vaults they own
CREATE POLICY vault_flags_access ON public.audit_flags
    FOR ALL USING (
        vault_id IN (SELECT id FROM public.vaults WHERE user_id = auth.uid())
    );

-- Policy: Users can see tasks in vaults they own
CREATE POLICY vault_tasks_access ON public.agent_tasks
    FOR ALL USING (
        vault_id IN (SELECT id FROM public.vaults WHERE user_id = auth.uid())
    );

-- Policy: Users can see outputs in vaults they own
CREATE POLICY vault_outputs_access ON public.generated_outputs
    FOR ALL USING (
        vault_id IN (SELECT id FROM public.vaults WHERE user_id = auth.uid())
    );

-- Policy: Users can see financial analyses in vaults they own
